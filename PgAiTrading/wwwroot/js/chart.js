// Candlestick chart renderer for PG AI Trading with zoom/pan support.
window.pgAiTradingChart = (function () {
    const states = {};
  ...
  // Blazor interop may send camelCase or PascalCase property names.
    ...
    function ensureInteractions(canvas, id) {
        ...
        function onWheel(e) {
            ...
            // Wheel up / pinch-out → zoom in (TradingView / laptop mouse).
            const factor = raw < 0 ? step : 1 / step;
            // Price-axis or Shift+wheel → vertical price zoom; else time zoom.
            // Ctrl/Meta+wheel (trackpad pinch) also time-zooms like TradingView.
        ...
        function beginDrag(e) {
            ...
            // TradingView-like modes:
            // - drag on price axis / Shift → vertical price scale
            // - left-drag on plot → free pan (time + price)
        ...
        function moveDrag(e) {
            ...
            // Free pan (TradingView): left-drag moves time (X) and price (Y).
            var didPan = false;
            if (dx !== 0) {
    ...
    window.pgAiTradingMergeOverlayInts = mergeOverlayInts;
...
window.pgAiTradingChartReady = function () {
    return window.pgAiTradingChart && typeof window.pgAiTradingChart.drawCandlestickChart === 'function';
};
...
window.pgAiTradingSetSt725Overlay = function (canvasId, show) {
    try {
        if (!window.pgAiTradingChartReady()) return false;
        return window.pgAiTradingChart.setSt725Overlay(canvasId, show);
    } catch (err) {
        console.error('pgAiTradingSetSt725Overlay failed', err);
        return false;
    }
};

window.pgAiTradingSetVwapOverlay = function (canvasId, show) {
    try {
        if (!window.pgAiTradingChartReady()) return false;
        return window.pgAiTradingChart.setVwapOverlay(canvasId, show);
    } catch (err) {
        console.error('pgAiTradingSetVwapOverlay failed', err);
        return false;
    }
};

window.pgAiTradingSetEma20Overlay = function (canvasId, show) {
    try {
        if (!window.pgAiTradingChartReady()) return false;
        return window.pgAiTradingChart.setEma20Overlay(canvasId, show);
    } catch (err) {
        console.error('pgAiTradingSetEma20Overlay failed', err);
        return false;
    }
};

window.pgAiTradingSetEmaOverlays = function (canvasId, showEma9, showEma20, showEma50, showEma200) {
    try {
        if (!window.pgAiTradingChartReady()) return false;
        return window.pgAiTradingChart.setEmaOverlays(canvasId, showEma9, showEma20, showEma50, showEma200);
    } catch (err) {
        console.error('pgAiTradingSetEmaOverlays failed', err);
        return false;
    }
};

window.pgAiTradingSetPatternsOverlay = function (canvasId, show) {
    try {
        if (!window.pgAiTradingChartReady()) return false;
        return window.pgAiTradingChart.setPatternsOverlay(canvasId, show);
    } catch (err) {
        console.error('pgAiTradingSetPatternsOverlay failed', err);
        return false;
    }
};

window.pgAiTradingSetIntradayCprOverlay = function (canvasId, show, segmentsJson) {
    try {
        if (!window.pgAiTradingChartReady()) return false;
        return window.pgAiTradingChart.setIntradayCprOverlay(canvasId, show, segmentsJson);
    } catch (err) {
        console.error('pgAiTradingSetIntradayCprOverlay failed', err);
        return false;
    }
};

window.pgAiTradingSetStudyOverlays = function (canvasId, showVwap, showEma20, showSt725, showSt103) {
    try {
        if (!window.pgAiTradingChartReady()) return false;
        return window.pgAiTradingChart.setStudyOverlays(canvasId, showVwap, showEma20, showSt725, showSt103);
    } catch (err) {
        console.error('pgAiTradingSetStudyOverlays failed', err);
        return false;
    }
};

window.pgAiTradingDrawCandles = function (
    canvasId,
    candlesJson,
    timeframe,
    levelsJson,
    ...
    try {
        ...
        overlayOptions = window.pgAiTradingMergeOverlayInts(overlayOptions, {
        ...
        return window.pgAiTradingChart.drawCandlestickChart(canvasId, candles, timeframe, levels, pocToday, overlayOptions);
    } catch (err) {
        console.error('pgAiTradingDrawCandles failed', err);
        return false;
    }
};
