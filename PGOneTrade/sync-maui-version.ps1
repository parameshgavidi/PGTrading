# Align PGOneTrade NuGet MAUI package versions with the installed MAUI workload.
...
$csprojPath = Join-Path $PSScriptRoot "PGOneTrade.csproj"
$csproj = Get-Content $csprojPath -Raw
$tfmMatch = [regex]::Match($csproj, '<TargetFramework>([^<]+)</TargetFramework>')
if (-not $tfmMatch.Success) {
    Write-Host "ERROR: Could not read TargetFramework from PGOneTrade.csproj" -ForegroundColor Red
    exit 1
}
...
if ($tfmMajor -ne 0 -and $tfmMajor -ne $mauiMajor) {
    ...
    Write-Host "  PGOneTrade.csproj targets: $tfm (expects MAUI $tfmMajor.x)"
    Write-Host "  Installed workload:   maui $version (MAUI $mauiMajor.x)"
    Write-Host ""
    Write-Host "Fix: update <TargetFramework> in PGOneTrade.csproj to net$mauiMajor.0-windows10.0.19041.0"
