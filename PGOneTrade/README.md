# PG One Trade — SuperTrend Nifty Trading Assistant
...
![PG One Trade](Resources/Images/pgonetrade_logo.png)
...
## Setup on D:\PGOneTrade

### Option 1: Clone this repository

```powershell
git clone <repo-url> D:\PGOneTrade
cd D:\PGOneTrade\PGOneTrade
```

### Option 2: Copy the PGOneTrade folder

Copy the entire `PGOneTrade` project folder to `D:\PGOneTrade`.
...
Or open `PG One Trade.sln` in **Visual Studio 2026 Community** and press **F5**.

### Install .NET 10 SDK

PG One Trade targets **.NET 10 / MAUI 10**. If `.\check-sdk.ps1` reports a missing SDK:
...
- Root `App.xaml` must be `x:Class="PGOneTrade.App"` inheriting from MAUI `Application`
- Windows `Platforms/Windows/App.xaml` must be `x:Class="PGOneTrade.WinUI.App"` inheriting from `MauiWinUIApplication`
- Do **not** set Windows App.xaml to `x:Class="PGOneTrade.App"` — that causes this error
...
4. Reopen `PG One Trade.sln` and press **F5**
...
```powershell
cd D:\PGOneTrade\parameshgavidi\PGTrading\PGOneTrade
...
| Screen | Meaning |
|--------|---------|
| Blue "Starting PG One Trade..." → "Loading PG One Trade UI…" → full UI | Working |
...
3. In PG One Trade, go to **Settings**
