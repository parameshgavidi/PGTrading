// Candlestick chart renderer for PG One Trade with zoom/pan support.
window.pgOneTradeChart = (function () {
    const states = {};
  ...
  // Blazor interop may send camelCase or PascalCase property names.
    ...
    window.pgOneTradeMergeOverlayInts = mergeOverlayInts;
...
window.pgOneTradeChartReady = function () {
    return window.pgOneTradeChart && typeof window.pgOneTradeChart.drawCandlestickChart === 'function';
};

window.pgOneTradeChartHasState = function (canvasId) {
    return window.pgOneTradeChart && window.pgOneTradeChart.hasState(canvasId);
};

window.pgOneTradeSetSt725Overlay = function (canvasId, show) {
    try {
        if (!window.pgOneTradeChartReady()) return false;
        return window.pgOneTradeChart.setSt725Overlay(canvasId, show);
    } catch (err) {
        console.error('pgOneTradeSetSt725Overlay failed', err);
        return false;
    }
};

window.pgOneTradeSetVwapOverlay = function (canvasId, show) {
    try {
        if (!window.pgOneTradeChartReady()) return false;
        return window.pgOneTradeChart.setVwapOverlay(canvasId, show);
    } catch (err) {
        console.error('pgOneTradeSetVwapOverlay failed', err);
        return false;
    }
};

window.pgOneTradeSetEma20Overlay = function (canvasId, show) {
    try {
        if (!window.pgOneTradeChartReady()) return false;
        return window.pgOneTradeChart.setEma20Overlay(canvasId, show);
    } catch (err) {
        console.error('pgOneTradeSetEma20Overlay failed', err);
        return false;
    }
};

window.pgOneTradeSetEmaOverlays = function (canvasId, showEma9, showEma20, showEma50, showEma200) {
    try {
        if (!window.pgOneTradeChartReady()) return false;
        return window.pgOneTradeChart.setEmaOverlays(canvasId, showEma9, showEma20, showEma50, showEma200);
    } catch (err) {
        console.error('pgOneTradeSetEmaOverlays failed', err);
        return false;
    }
};

window.pgOneTradeSetPatternsOverlay = function (canvasId, show) {
    try {
        if (!window.pgOneTradeChartReady()) return false;
        return window.pgOneTradeChart.setPatternsOverlay(canvasId, show);
    } catch (err) {
        console.error('pgOneTradeSetPatternsOverlay failed', err);
        return false;
    }
};

window.pgOneTradeSetIntradayCprOverlay = function (canvasId, show, segmentsJson) {
    try {
        if (!window.pgOneTradeChartReady()) return false;
        return window.pgOneTradeChart.setIntradayCprOverlay(canvasId, show, segmentsJson);
    } catch (err) {
        console.error('pgOneTradeSetIntradayCprOverlay failed', err);
        return false;
    }
};

window.pgOneTradeSetStudyOverlays = function (canvasId, showVwap, showEma20, showSt725, showSt103) {
    try {
        if (!window.pgOneTradeChartReady()) return false;
        return window.pgOneTradeChart.setStudyOverlays(canvasId, showVwap, showEma20, showSt725, showSt103);
    } catch (err) {
        console.error('pgOneTradeSetStudyOverlays failed', err);
        return false;
    }
};

window.pgOneTradeDrawCandles = function (
    canvasId,
    candlesJson,
    timeframe,
    levelsJson,
    ...
    try {
        ...
        overlayOptions = window.pgOneTradeMergeOverlayInts(overlayOptions, {
        ...
        return window.pgOneTradeChart.drawCandlestickChart(canvasId, candles, timeframe, levels, pocToday, overlayOptions);
    } catch (err) {
        console.error('pgOneTradeDrawCandles failed', err);
        return false;
    }
};
